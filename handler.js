...
if (!isNumber(user.money)) user.money = false
...
// this.reply(m.chat, `🫥 No tienes oggy coins`, m)
continue     
}
m.exp += xp
...
if (m.money)  
m.reply(+m.money + ' OGGYCOINS USADOS')
break
}}
} catch (e) {
console.error(e)
...
if (!m.fromMem && m.text.match(/(@930982220|admin del bot|Oggy_mods|:v)/gi)) {
...
(chat.sBye || this.bye || conn.bye || 'adios Bye, @user!')).replace('@user', '@' + user.split('@')[0])
